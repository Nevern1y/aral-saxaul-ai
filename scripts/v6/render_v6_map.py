...
DISPLAY_NAMES_RU = {
    1: "Перспективная зона (низкая засолённость)",
    3: "Умеренная засолённость",
    4: "Высокая засолённость",
    10: "Существующая растительность",
    0: "Водоёмы / отсутствуют данные",
}
...
def metric_note(metrics: dict) -> str:
    ...
    return f"Пространственное разрешение — 30 м. Модель оценки засолённости построена на основе {n_txt} ({auc_txt})"
...
<div id="decision-help" style="position:fixed; top:12px; right:12px; z-index:9999;
    ...
    <div style="font-weight:700; font-size:12.5px; margin-bottom:3px; color:#0F172A;">Информация о выбранном участке</div>
    <div id="decision-main" style="color:#334155;">
        Наведите курсор на окрашенную область в слое V6.
    </div>
    <div id="decision-detail" style="margin-top:5px; color:#64748B; font-size:11px;">
        Прозрачные области не используются при оценке.
    </div>
    <div style="margin-top:7px; color:#94A3B8; font-size:10px; border-top:1px solid #E2E8F0; padding-top:5px;">
        {metric_note(metrics)}. Предназначена для предварительной оценки и не является основанием для принятия решения о посадке.
    </div>
</div>
"""
...
(function() {{
  ...
  const bounds = {{south:{south:.12f}, west:{west:.12f}, north:{north:.12f}, east:{east:.12f}}};
    const zoneInfo = {{
    1: {{
      title: 'Рекомендуется для полевого обследования',
      label: 'Перспективная зона с низкой засолённостью',
      advice: 'Сначала оцените доступность участка и выполните отбор почвенных проб. Если результаты подтвердят низкую засолённость, участок может быть включён в число приоритетных объектов для проведения посадок.',
      color: '#065F46'
    }},
    3: {{
      title: 'Требуется дополнительная проверка',
      label: 'Умеренный риск засоления',
      advice: 'Не рекомендуется проводить посадку без предварительного отбора и анализа почвенных проб. Участок может рассматриваться как резервный вариант либо использоваться для опытных посадок после подтверждения благоприятных почвенных условий.',
      color: '#B45309'
    }},
    4: {{
      title: 'Высокий риск засоления',
      label: 'Высокая засолённость',
      advice: 'Участок малопригоден для посадки саксаула без проведения мероприятий по снижению засолённости почвы или применения специальных технологий восстановления. Рекомендуется не включать данный участок в план первоочередных полевых обследований.',
      color: '#991B1B'
    }},
    10: {{
      title: 'Участок уже покрыт растительностью',
      label: 'Существующая растительность (не предназначен для новых посадок)',
      advice: 'На участке уже имеется растительный покров. Его рекомендуется использовать в качестве контрольного участка для оценки текущего состояния экосистемы, а не для проведения новых посадок.',
      color: '#047857'
    }}
  }};

  function describeScore(score) {{
    if (score >= 0.66) return 'высокая вероятность низкой засолённости';
    if (score >= 0.33) return 'средняя вероятность низкой засолённости';
    return 'низкая вероятность низкой засолённости';
  }}

  function setEmpty() {{
    panelMain.innerHTML = 'Для данной точки оценка отсутствует.';
    panelDetail.innerHTML = 'Наведите курсор на окрашенную область в слое V6. Прозрачные области не используются при оценке.';
  }}
  ...
  function update(latlng) {{
    ...
    panelMain.innerHTML = '<div style="font-weight:700;color:' + info.color + ';">' + info.title + '</div>' +
      ...
      '<div style="margin-top:6px;"><b>Индекс благоприятности участка:</b> ' + pct + ' из 100 (' + scoreText + ')</div>' +
      '<div><b>Предполагаемый риск засоления:</b> ' + saline + ' из 100 по модели V6</div>' +
      '<div><b>Класс участка:</b> ' + info.label + '</div>';
    panelDetail.innerHTML = '<b>Рекомендация:</b> ' + info.advice +
...
"""
    ...
    <div style="margin-top:7px; padding-top:5px; border-top:1px solid #E2E8F0;
        ...
        Карта используется для предварительной оценки риска засоления. Зоны 3 и 4
        показывают разные уровни единой шкалы риска, а не окончательные категории пригодности.
    </div>
</div>
"""
